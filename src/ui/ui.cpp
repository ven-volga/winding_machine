// ui.cpp
//
// Макет екрану (4×20):
//
//   Col: 01234567890123456789
//   Row 0: >SPD:300rpm  >FWD>>>
//   Row 1: >TURNS:9999/0000
//   Row 2: >POS:  0.00mm AR:+
//   Row 3: >DIA:0.20  >W: 30.0
//
// Row 0: SPD (col 0), FWD/REV (col 12, курсор col 11)  ← зсунуто на 1
// Row 1: TURNS задані / намотані (курсор col 0)
// Row 2: POS + AR:+/- або >>> / <<< в сервісі
// Row 3: DIA (col 0), W (col 11, курсор col 10)        ← зсунуто на 1
//
// TURNS редагування:
//   Повільно (>200мс між кроками) → крок 1
//   Середньо (>100мс)             → крок 10
//   Швидко   (<100мс)             → крок 100
//
// Підменю POS:
//   Крутиш         → обираєш пункт
//   Довгий натиск  → входиш в редагування
//   Короткий натиск → вихід в головне меню
//   В редагуванні:
//   Крутиш          → змінюєш значення
//   Довгий натиск   → зберегти і вийти
//   Короткий натиск → скасувати

#include "ui.h"
#include "encoder.h"
#include "lcd.h"
#include "shared_state.h"
#include "config.h"
#include "motion.h"

#include <cstdio>
#include <cmath>

extern "C" {
    #include "freertos/FreeRTOS.h"
    #include "freertos/task.h"
    #include "esp_timer.h"
}

// ─────────────────────────────────────────
// Режими
// ─────────────────────────────────────────

typedef enum
{
    UI_VIEW,
    UI_EDIT,
    UI_SUBMENU_POS,
    UI_SUBMENU_EDIT,
    UI_SUBMENU_TURNS,
    UI_SUBMENU_TURNS_EDIT,
} UiMode;

typedef enum
{
    PARAM_SPEED     = 0,
    PARAM_DIRECTION = 1,
    PARAM_TURNS     = 2,
    PARAM_POS       = 3,
    PARAM_DIAMETER  = 4,
    PARAM_WIDTH     = 5,
    PARAM_COUNT     = 6,
} Param;

typedef enum
{
    SUBMENU_GO_HOME   = 0,  // поїхати в позицію 0
    SUBMENU_SET_VALUE = 1,  // задати позицію і поїхати
    SUBMENU_SET_HOME  = 2,  // скинути поточну позицію в 0
    SUBMENU_AUTO_REV  = 3,  // автореверс ON/OFF
    SUBMENU_COUNT     = 4,
} PosSubmenu;

typedef enum
{
    TURNS_SET   = 0,  // задати кількість витків (з прискоренням)
    TURNS_RESET = 1,  // скинути лічильник в 0
    TURNS_COUNT = 2,
} TurnsSubmenu;

// ─────────────────────────────────────────
// Стан UI
// ─────────────────────────────────────────

static UiMode  uiMode         = UI_VIEW;
static uint8_t selectedParam  = PARAM_SPEED;
static bool    needFullRedraw  = true;

// Буфер редагування
static uint32_t editSpeed      = 0;
static float    editDia        = 0.0f;
static float    editWidth      = 0.0f;
static bool     editDir        = true;
static uint32_t editTargetTurns = 0;

// Підменю POS
static uint8_t  submenuItem  = SUBMENU_SET_VALUE;
static float    editPos      = 0.0f;
static bool     editAutoRev  = true;

// Підменю TURNS
static uint8_t turnsItem = TURNS_SET;

// Кеш
static uint32_t lastTurns        = 0xFFFFFFFF;
static uint32_t lastTargetTurns  = 0xFFFFFFFF;
static float    lastPos          = -9999.0f;
static uint32_t lastSpeed        = 0xFFFFFFFF;
static bool     lastDir          = true;
static bool     lastSvcActive    = false;
static bool     lastSvcDir       = true;
static bool     lastAutoRev      = true;

// Для прискорення TURNS
static uint32_t lastTurnsEditMs  = 0;

// Позиції курсорів {col, row}
static const uint8_t CURSOR_POS[PARAM_COUNT][2] =
{
    {0,  0},   // SPEED
    {12, 0},   // DIRECTION (курсор col 12, текст col 13)
    {0,  1},   // TURNS
    {0,  2},   // POS
    {0,  3},   // DIAMETER
    {11, 3},   // WIDTH (курсор col 11, текст col 12)
};

// ─────────────────────────────────────────
// Таймер енкодера (1мс)
// ─────────────────────────────────────────

static esp_timer_handle_t encoderTimer;

// Таймер ТІЛЬКИ читає енкодер і накопичує кроки в encSteps.
// Ніякої бізнес-логіки, ніяких mutex, ніяких shared.
// Вся обробка — в ui_update().
static void IRAM_ATTR encoder_timer_cb(void* arg)
{
    encoder_update();
}

// ─────────────────────────────────────────
// Малювання
// ─────────────────────────────────────────

static void draw_speed(uint32_t spd)
{
    char buf[11];
    snprintf(buf, sizeof(buf), "SPD:%3lurpm", (unsigned long)spd);
    lcd_set_cursor(1, 0);
    lcd_print(buf);
    lastSpeed = spd;
}

// FWD/REV (курсор col 12, текст col 13)
static void draw_direction(bool fwd)
{
    lcd_set_cursor(13, 0);
    lcd_print(fwd ? "FWD>>>" : "REV<<<");
    lastDir = fwd;
}

// TURNS: задані / намотані
static void draw_turns(uint32_t turns, uint32_t target)
{
    char buf[21];
    if (target == 0)
    {
        uint32_t t = turns > 9999 ? 9999 : turns;
        snprintf(buf, sizeof(buf), " TURNS:----%04u    ", (unsigned)t);
    }
    else
    {
        uint32_t t = turns  > 9999 ? 9999 : turns;
        uint32_t g = target > 9999 ? 9999 : target;
        snprintf(buf, sizeof(buf), " TURNS:%04u/%04u   ", (unsigned)g, (unsigned)t);
    }
    buf[20] = '\0';
    lcd_set_cursor(0, 1);
    lcd_print(buf);
    lastTurns       = turns;
    lastTargetTurns = target;
}

// Row 2: POS + AR:+/- або >>> / <<< в сервісі
static void draw_position_row(float pos, bool svcActive, bool svcDir, bool autoRev)
{
    char info[5];
    if (svcActive)
        snprintf(info, sizeof(info), svcDir ? ">>> " : "<<< ");
    else
        snprintf(info, sizeof(info), autoRev ? "AR:+" : "AR:-");

    char buf[21];
    snprintf(buf, sizeof(buf), " POS:%6.2fmm %s", pos, info);
    buf[20] = '\0';
    lcd_set_cursor(0, 2);
    lcd_print(buf);
    lastPos       = pos;
    lastSvcActive = svcActive;
    lastSvcDir    = svcDir;
    lastAutoRev   = autoRev;
}

static void draw_diameter(float dia)
{
    char buf[10];
    snprintf(buf, sizeof(buf), "DIA:%.2f ", dia);
    lcd_set_cursor(1, 3);
    lcd_print(buf);
}

// W (курсор col 11, текст col 12)
static void draw_width(float w)
{
    char buf[9];
    snprintf(buf, sizeof(buf), "W:%5.1f", w);
    lcd_set_cursor(12, 3);
    lcd_print(buf);
}

static void draw_cursor(uint8_t prev)
{
    lcd_set_cursor(CURSOR_POS[prev][0], CURSOR_POS[prev][1]);
    lcd_print(" ");
    lcd_set_cursor(CURSOR_POS[selectedParam][0], CURSOR_POS[selectedParam][1]);
    lcd_print(uiMode == UI_EDIT ? "*" : ">");
}

static void draw_full_screen(const SharedState& s)
{
    lcd_clear();
    lastTurns       = 0xFFFFFFFF;
    lastTargetTurns = 0xFFFFFFFF;
    lastPos         = -9999.0f;
    lastSpeed       = 0xFFFFFFFF;
    lastDir         = !s.dirForward;
    lastSvcActive   = false;
    lastAutoRev     = !s.autoReverse;

    draw_speed(s.spindleSpeed);
    draw_direction(s.dirForward);
    draw_turns(s.turns, s.targetTurns);
    draw_position_row(s.carriagePos, s.serviceCarriageActive,
                      s.serviceCarriageDir, s.autoReverse);
    draw_diameter(s.wireDiameter);
    draw_width(s.windingWidth);

    lcd_set_cursor(CURSOR_POS[selectedParam][0], CURSOR_POS[selectedParam][1]);
    lcd_print(">");
}

// ─────────────────────────────────────────
// Підменю POS
// ─────────────────────────────────────────

// ─────────────────────────────────────────
// Підменю TURNS — 2 рядки + 2 порожніх (центровані)
// ─────────────────────────────────────────

static void draw_submenu_turns(bool editMode, uint32_t currentTurns, uint32_t targetTurns)
{
    lcd_clear();
    char buf[21];
    char cur;

    // Row 0: заголовок
    lcd_set_cursor(0, 0); lcd_print("-- TURNS MENU --    ");

    // Row 1: Set target
    cur = (turnsItem == TURNS_SET) ? (editMode ? '*' : '>') : ' ';
    if (editMode && turnsItem == TURNS_SET)
        snprintf(buf, sizeof(buf), "%cTarget:%4u turns  ", cur, (unsigned)(targetTurns > 9999 ? 9999 : targetTurns));
    else
    {
        uint32_t t = targetTurns > 9999 ? 9999 : targetTurns;
        snprintf(buf, sizeof(buf), "%cSet target: %4u   ", cur, (unsigned)t);
    }
    buf[20] = '\0';
    lcd_set_cursor(0, 1); lcd_print(buf);

    // Row 2: Reset counter
    cur = (turnsItem == TURNS_RESET) ? (editMode ? '*' : '>') : ' ';
    uint32_t ct = currentTurns > 9999 ? 9999 : currentTurns;
    snprintf(buf, sizeof(buf), "%cReset  now:%4u    ", cur, (unsigned)ct);
    buf[20] = '\0';
    lcd_set_cursor(0, 2); lcd_print(buf);

    // Row 3: підказка
    lcd_set_cursor(0, 3);
    lcd_print(" Long=ok  Short=back");
}

static void update_turns_cursor(uint8_t prev, bool editMode, uint32_t currentTurns, uint32_t targetTurns)
{
    // Перемальовуємо тільки 2 рядки
    char buf[21];
    char cur;

    auto draw_row = [&](uint8_t idx, uint8_t row) {
        cur = (turnsItem == idx) ? (editMode ? '*' : '>') : ' ';
        if (idx == TURNS_SET)
        {
            uint32_t t = targetTurns > 9999 ? 9999 : targetTurns;
            snprintf(buf, sizeof(buf), "%cSet target: %4u   ", cur, (unsigned)t);
        }
        else
        {
            uint32_t ct = currentTurns > 9999 ? 9999 : currentTurns;
            snprintf(buf, sizeof(buf), "%cReset  now:%4u    ", cur, (unsigned)ct);
        }
        buf[20] = '\0';
        lcd_set_cursor(0, row);
        lcd_print(buf);
    };

    draw_row(prev == TURNS_SET ? TURNS_SET : TURNS_RESET,
             prev == TURNS_SET ? 1 : 2);
    draw_row(turnsItem == TURNS_SET ? TURNS_SET : TURNS_RESET,
             turnsItem == TURNS_SET ? 1 : 2);
}

// ─────────────────────────────────────────
// Намалювати один рядок підменю
static void draw_submenu_row(uint8_t row, uint8_t idx, bool editMode)
{
    char buf[21];
    char cur = (submenuItem == idx) ? (editMode ? '*' : '>') : ' ';

    switch (idx)
    {
        case SUBMENU_GO_HOME:
            snprintf(buf, sizeof(buf), "%cGo home -> 0.0mm  ", cur);
            break;
        case SUBMENU_SET_VALUE:
            snprintf(buf, sizeof(buf), "%cSet pos: %6.1fmm", cur, editPos);
            break;
        case SUBMENU_SET_HOME:
            snprintf(buf, sizeof(buf), "%cSet home  0.00mm  ", cur);
            break;
        case SUBMENU_AUTO_REV:
            snprintf(buf, sizeof(buf), "%cAuto rev:     %s",
                     cur, editAutoRev ? "ON " : "OFF");
            break;
        default:
            buf[0] = '\0';
            break;
    }
    buf[20] = '\0';
    lcd_set_cursor(0, row);
    lcd_print(buf);
}

static void draw_submenu_pos(const SharedState& s, bool editMode)
{
    lcd_clear();
    // 4 пункти на 4 рядки — без заголовка
    draw_submenu_row(0, SUBMENU_GO_HOME,   editMode);
    draw_submenu_row(1, SUBMENU_SET_VALUE, editMode);
    draw_submenu_row(2, SUBMENU_SET_HOME,  editMode);
    draw_submenu_row(3, SUBMENU_AUTO_REV,  editMode);
}

// Оновити тільки два рядки курсора (попередній і новий)
// без перемалювання всього екрану
static void update_submenu_cursor(uint8_t prev, bool editMode)
{
    draw_submenu_row(prev,        prev,        editMode);
    draw_submenu_row(submenuItem, submenuItem, editMode);
}

// ─────────────────────────────────────────
// Публічні функції
// ─────────────────────────────────────────

void ui_init()
{
    lcd_init();
    encoder_init();

    esp_timer_create_args_t timerArgs = {
        .callback              = encoder_timer_cb,
        .arg                   = nullptr,
        .dispatch_method       = ESP_TIMER_TASK,
        .name                  = "enc_timer",
        .skip_unhandled_events = true,
    };
    esp_timer_create(&timerArgs, &encoderTimer);
    esp_timer_start_periodic(encoderTimer, 1000);

    needFullRedraw = true;
}

void ui_update()
{
    SharedState s;
    shared_lock();
    s = shared;
    shared_unlock();

    // Читаємо і скидаємо прапорці кнопки одразу
    // Це гарантує що кожна подія обробляється тільки один раз
    // навіть якщо таймер встиг викликати encoder_update кілька разів
    bool click    = evClick;
    bool longPress = evLong;
    evClick = false;
    evLong  = false;

    if (needFullRedraw)
    {
        if (uiMode == UI_SUBMENU_POS || uiMode == UI_SUBMENU_EDIT)
            draw_submenu_pos(s, uiMode == UI_SUBMENU_EDIT);
        else if (uiMode == UI_SUBMENU_TURNS || uiMode == UI_SUBMENU_TURNS_EDIT)
            draw_submenu_turns(uiMode == UI_SUBMENU_TURNS_EDIT,
                               s.turns, editTargetTurns);
        else
            draw_full_screen(s);
        needFullRedraw = false;
        return;
    }

    // ════════════════════════════════════════════════════════════
    // VIEW
    // ════════════════════════════════════════════════════════════
    if (uiMode == UI_VIEW)
    {
        // Читаємо і скидаємо накопичувач атомарно
        int32_t steps = encSteps;
        encSteps = 0;

        // Під час намотки — крутіння міняє швидкість напряму
        if (steps != 0 && s.running)
        {
            shared_lock();
            uint32_t spd = shared.spindleSpeed;
            if (steps > 0 && spd < SPINDLE_SPEED_MAX) spd += SPINDLE_SPEED_STEP;
            if (steps < 0 && spd > SPINDLE_SPEED_MIN) spd -= SPINDLE_SPEED_STEP;
            shared.spindleSpeed = spd;
            shared_unlock();
            steps = 0;
        }

        if (steps != 0 && !s.running)
        {
            uint8_t prev = selectedParam;
            if (steps > 0)
                selectedParam = (selectedParam + 1) % PARAM_COUNT;
            else
                selectedParam = (selectedParam + PARAM_COUNT - 1) % PARAM_COUNT;
            draw_cursor(prev);
        }

        if (longPress)
        {
            if (selectedParam == PARAM_POS)
            {
                shared_lock();
                editPos     = shared.carriagePos;
                editAutoRev = shared.autoReverse;
                shared_unlock();
                submenuItem    = SUBMENU_SET_VALUE;
                uiMode         = UI_SUBMENU_POS;
                needFullRedraw = true;
            }
            else if (selectedParam == PARAM_TURNS)
            {
                turnsItem      = TURNS_SET;
                uiMode         = UI_SUBMENU_TURNS;
                needFullRedraw = true;
            }
            else
            {
                editSpeed       = s.spindleSpeed;
                editDia         = s.wireDiameter;
                editWidth       = s.windingWidth;
                editDir         = s.dirForward;
                editTargetTurns = s.targetTurns;
                uiMode = UI_EDIT;
                lcd_set_cursor(CURSOR_POS[selectedParam][0],
                               CURSOR_POS[selectedParam][1]);
                lcd_print("*");
            }
        }

        // Динамічні поля
        if (s.spindleSpeed != lastSpeed)
            draw_speed(s.spindleSpeed);
        if (s.dirForward != lastDir)
            draw_direction(s.dirForward);
        if (s.turns != lastTurns || s.targetTurns != lastTargetTurns)
            draw_turns(s.turns, s.targetTurns);
        if (fabsf(s.carriagePos - lastPos) > 0.005f    ||
            s.serviceCarriageActive != lastSvcActive    ||
            s.serviceCarriageDir    != lastSvcDir       ||
            s.autoReverse           != lastAutoRev)
        {
            draw_position_row(s.carriagePos, s.serviceCarriageActive,
                              s.serviceCarriageDir, s.autoReverse);
        }
    }

    // ════════════════════════════════════════════════════════════
    // EDIT
    // ════════════════════════════════════════════════════════════
    else if (uiMode == UI_EDIT)
    {
        // Читаємо накопичувач (в VIEW вже міг скинути, тут свіжий)
        int32_t steps = encSteps;
        encSteps = 0;

        if (steps != 0)
        {
            uint32_t now = (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS);

            switch (selectedParam)
            {
                case PARAM_SPEED:
                    if (steps > 0 && editSpeed < SPINDLE_SPEED_MAX)
                        editSpeed += SPINDLE_SPEED_STEP;
                    if (steps < 0 && editSpeed > SPINDLE_SPEED_MIN)
                        editSpeed -= SPINDLE_SPEED_STEP;
                    draw_speed(editSpeed);
                    break;

                case PARAM_DIRECTION:
                    editDir = !editDir;
                    draw_direction(editDir);
                    break;

                case PARAM_TURNS:
                {
                    // Прискорення залежно від швидкості кручення
                    uint32_t elapsed = now - lastTurnsEditMs;
                    uint32_t step = 1;
                    if      (elapsed < 50)  step = 100;
                    else if (elapsed < 100) step = 10;
                    lastTurnsEditMs = now;

                    if (steps > 0)
                        editTargetTurns = (editTargetTurns + step > 9999) ?
                                           9999 : editTargetTurns + step;
                    else
                        editTargetTurns = (editTargetTurns < step) ?
                                           0 : editTargetTurns - step;

                    // Оновлюємо тільки поле TURNS
                    char buf[21];
                    if (editTargetTurns == 0)
                    {
                        uint32_t st = s.turns > 9999 ? 9999 : s.turns;
                        snprintf(buf, sizeof(buf), "*TURNS:----%04u    ", (unsigned)st);
                    }
                    else
                    {
                        uint32_t et = editTargetTurns > 9999 ? 9999 : editTargetTurns;
                        uint32_t st = s.turns > 9999 ? 9999 : s.turns;
                        snprintf(buf, sizeof(buf), "*TURNS:%04u/%04u   ",
                                 (unsigned)et, (unsigned)st);
                    }
                    buf[20] = '\0';
                    lcd_set_cursor(0, 1);
                    lcd_print(buf);
                    break;
                }

                case PARAM_DIAMETER:
                    if (steps > 0 && editDia < WIRE_DIAMETER_MAX)
                        editDia += WIRE_DIAMETER_STEP;
                    if (steps < 0 && editDia > WIRE_DIAMETER_MIN)
                        editDia -= WIRE_DIAMETER_STEP;
                    draw_diameter(editDia);
                    break;

                case PARAM_WIDTH:
                    if (steps > 0 && editWidth < WINDING_WIDTH_MAX)
                        editWidth += WINDING_WIDTH_STEP;
                    if (steps < 0 && editWidth > WINDING_WIDTH_MIN)
                        editWidth -= WINDING_WIDTH_STEP;
                    draw_width(editWidth);
                    break;
            }
            
        }

        if (click)
        {
            uiMode = UI_VIEW;
            needFullRedraw = true;
        }

        if (longPress)
        {
            shared_lock();
            shared.spindleSpeed  = editSpeed;
            shared.wireDiameter  = editDia;
            shared.windingWidth  = editWidth;
            shared.dirForward    = editDir;
            shared.targetTurns   = editTargetTurns;
            shared_unlock();
            uiMode = UI_VIEW;
            needFullRedraw = true;
        }
    }

    // ════════════════════════════════════════════════════════════
    // SUBMENU_POS — вибір пункту (крутиш без перемалювання)
    // ════════════════════════════════════════════════════════════
    else if (uiMode == UI_SUBMENU_POS)
    {
        int32_t steps = encSteps;
        encSteps = 0;

        if (steps != 0)
        {
            uint8_t prev = submenuItem;
            if (steps > 0)
                submenuItem = (submenuItem + 1) % SUBMENU_COUNT;
            else
                submenuItem = (submenuItem + SUBMENU_COUNT - 1) % SUBMENU_COUNT;
            // Оновлюємо тільки 2 рядки — без блимання
            update_submenu_cursor(prev, false);
        }

        if (click)
        {
            uiMode = UI_VIEW;
            needFullRedraw = true;
        }

        if (longPress)
        {
            uiMode = UI_SUBMENU_EDIT;
            needFullRedraw = true;
        }
    }

    // ════════════════════════════════════════════════════════════
    // SUBMENU_EDIT — редагування пункту
    // ════════════════════════════════════════════════════════════
    else if (uiMode == UI_SUBMENU_EDIT)
    {
        int32_t steps = encSteps;
        encSteps = 0;

        if (steps != 0)
        {
            switch (submenuItem)
            {
                case SUBMENU_SET_VALUE:
                    if (steps > 0 && editPos < CARRIAGE_POS_MAX)
                        editPos += CARRIAGE_POS_STEP;
                    if (steps < 0 && editPos > CARRIAGE_POS_MIN)
                        editPos -= CARRIAGE_POS_STEP;
                    // Оновлюємо тільки цей рядок — без блимання
                    draw_submenu_row(1, SUBMENU_SET_VALUE, true);
                    break;
                case SUBMENU_AUTO_REV:
                    editAutoRev = !editAutoRev;
                    draw_submenu_row(3, SUBMENU_AUTO_REV, true);
                    break;
                case SUBMENU_GO_HOME:
                case SUBMENU_SET_HOME:
                    // Крутіння не потрібне
                    break;
            }
        }

        // Короткий натиск → скасувати, повернутись в підменю
        if (click)
        {
            shared_lock();
            editPos     = shared.carriagePos;
            editAutoRev = shared.autoReverse;
            shared_unlock();
            uiMode = UI_SUBMENU_POS;
            needFullRedraw = true;
        }

        // Довгий натиск → виконати дію
        if (longPress)
        {
            switch (submenuItem)
            {
                case SUBMENU_GO_HOME:
                    // Поїхати в позицію 0 без зміни home point
                    motion_move_to(0.0f);
                    uiMode = UI_VIEW;
                    needFullRedraw = true;
                    break;

                case SUBMENU_SET_VALUE:
                    // Поїхати до заданої позиції
                    motion_move_to(editPos);
                    uiMode = UI_VIEW;
                    needFullRedraw = true;
                    break;

                case SUBMENU_SET_HOME:
                    // Скинути поточну позицію в 0 (не рухатись)
                    shared_lock();
                    shared.carriagePos = 0.0f;
                    shared_unlock();
                    lcd_clear();
                    lcd_set_cursor(3, 1); lcd_print("HOME SET!");
                    lcd_set_cursor(2, 2); lcd_print("pos = 0.00mm");
                    vTaskDelay(pdMS_TO_TICKS(1000));
                    uiMode = UI_VIEW;
                    needFullRedraw = true;
                    break;

                case SUBMENU_AUTO_REV:
                    shared_lock();
                    shared.autoReverse = editAutoRev;
                    shared_unlock();
                    uiMode = UI_SUBMENU_POS;
                    needFullRedraw = true;
                    break;
            }
        }
    }
    // ════════════════════════════════════════════════════════════
    // SUBMENU_TURNS — вибір пункту
    // ════════════════════════════════════════════════════════════
    else if (uiMode == UI_SUBMENU_TURNS)
    {
        int32_t steps = encSteps;
        encSteps = 0;

        if (steps != 0)
        {
            uint8_t prev = turnsItem;
            if (steps > 0)
                turnsItem = (turnsItem + 1) % TURNS_COUNT;
            else
                turnsItem = (turnsItem + TURNS_COUNT - 1) % TURNS_COUNT;
            update_turns_cursor(prev, false, s.turns, editTargetTurns);
        }

        // Короткий натиск → вийти в головне меню
        if (click)
        {
            uiMode = UI_VIEW;
            needFullRedraw = true;
        }

        // Довгий натиск → увійти в редагування
        if (longPress)
        {
            shared_lock();
            editTargetTurns = shared.targetTurns;
            shared_unlock();
            uiMode = UI_SUBMENU_TURNS_EDIT;
            needFullRedraw = true;
        }
    }

    // ════════════════════════════════════════════════════════════
    // SUBMENU_TURNS_EDIT — редагування пункту
    // ════════════════════════════════════════════════════════════
    else if (uiMode == UI_SUBMENU_TURNS_EDIT)
    {
        int32_t steps = encSteps;
        encSteps = 0;

        if (steps != 0 && turnsItem == TURNS_SET)
        {
            // Прискорення: чим швидше крутиш тим більший крок
            uint32_t now = (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS);
            uint32_t elapsed = now - lastTurnsEditMs;
            uint32_t step = 1;
            if      (elapsed < 50)  step = 100;
            else if (elapsed < 100) step = 10;
            lastTurnsEditMs = now;

            if (steps > 0)
                editTargetTurns = (editTargetTurns + step > 9999) ?
                                   9999 : editTargetTurns + step;
            else
                editTargetTurns = (editTargetTurns < step) ?
                                   0 : editTargetTurns - step;

            // Оновлюємо тільки рядок значення — без блимання
            char buf[21];
            snprintf(buf, sizeof(buf), "*Target:%4u turns  ",
                     (unsigned)(editTargetTurns > 9999 ? 9999 : editTargetTurns));
            buf[20] = '\0';
            lcd_set_cursor(0, 1); lcd_print(buf);
        }

        // Короткий натиск → скасувати
        if (click)
        {
            shared_lock();
            editTargetTurns = shared.targetTurns;
            shared_unlock();
            uiMode = UI_SUBMENU_TURNS;
            needFullRedraw = true;
        }

        // Довгий натиск → виконати
        if (longPress)
        {
            switch (turnsItem)
            {
                case TURNS_SET:
                    shared_lock();
                    shared.targetTurns = editTargetTurns;
                    shared_unlock();
                    uiMode = UI_VIEW;
                    needFullRedraw = true;
                    break;

                case TURNS_RESET:
                    shared_lock();
                    shared.turns      = 0;
                    shared.targetTurns = 0;
                    editTargetTurns   = 0;
                    shared_unlock();
                    // Підтвердження на екрані
                    lcd_clear();
                    lcd_set_cursor(2, 1); lcd_print("TURNS RESET!");
                    lcd_set_cursor(2, 2); lcd_print("counter = 0");
                    lcd_set_cursor(2, 3); lcd_print("target  = 0");
                    vTaskDelay(pdMS_TO_TICKS(1000));
                    uiMode = UI_VIEW;
                    needFullRedraw = true;
                    break;
            }
        }
    }

}